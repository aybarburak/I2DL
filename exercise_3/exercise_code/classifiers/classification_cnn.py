...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.pool = nn.MaxPool2d(pool, stride_pool)
        self.dropout = dropout
        self.hidden_dim = hidden_dim

        padding = int((kernel_size - 1) / 2)

        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride_conv, padding, bias=True)
        ...
        self.fc1 = nn.Linear(int(widthOut / pool) * int(heightOut / pool) * num_filters, hidden_dim, bias=True)
        # self.fc1 = nn.Linear((width // pool) * (height // pool) * num_filters, hidden_dim, bias=True)
