...
class KeypointModel(nn.Module):
    ...
    def __init__(self):
        ...
        ##############################################################################################################
        features = models.vgg11(pretrained=True).features
        features[0] = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=1, padding=1)
        
        self.features = nn.Sequential(*features)

        self.classifier = nn.Sequential(nn.Linear(512*3*3, 4096),
                                        nn.Linear(4096, 30))
        ########################################################################
        #                             END OF YOUR CODE                         #
        ########################################################################
