...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, dl_s = softmax_loss(scores, y)
        loss += 0.5 * self.reg * (np.sum(self.params['W1']**2) + np.sum(self.params['W2']**2)) 

        dx2, dw2, db2 = affine_backward(dl_s, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        out, relu_cache, dropout_cache = {}, {}, {}
        out = X
        for i in range(1, self.num_layers):
            out, relu_cache[i] = affine_relu_forward(out, self.params['W' + str(i)], self.params['b' + str(i)])
            if self.use_dropout:
                out, dropout_cache[i] = dropout_forward(out, self.dropout_param)

        scores, relu_cache[i + 1] = affine_forward(out, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx, dw, db = affine_backward(ds, relu_cache[self.num_layers])
        ...
        for i in range(1, self.num_layers ):
            if self.use_dropout:
                dx = dropout_backward(dx, dropout_cache[t - i])

            dx, dw, db = affine_relu_backward(dx, relu_cache[t - i])
            grads['W' + str(t - i)] = dw + self.reg * self.params['W' + str(t - i)]
            grads['b' + str(t - i)] = db
            loss += 0.5 * self.reg * (np.sum(self.params['W' + str(t - i)]**2))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
