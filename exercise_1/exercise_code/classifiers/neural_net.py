...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # array of shape (N, C).                                               #         
        ########################################################################
        layer_1_scores = np.dot(X, W1) + b1
        layer_1 = np.maximum(layer_1_scores, 0)
        scores = np.dot(layer_1, W2) + b2 
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        loss = (loss / N) + reg * (np.sum(W1**2) + np.sum(W2**2)) * 2.
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        grads['W2'] = layer_1.T.dot(dW2) / N + reg * W2
        grads['b2'] = np.ones(N).dot(dW2) / N

        dW1 = np.dot(dW2, W2.T)
        dW1[layer_1 <= 0] = 0

        grads['W1'] = np.dot(X.T, dW1) / N + reg * W1
        grads['b1'] = np.ones(N).dot(dW1) / N
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
    ...
    def predict(self, X):
        ...
        ########################################################################
        h = np.maximum(np.dot(X, self.params['W1']) + self.params['b1'], 0)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    hidden_size = 256
    results = {}
    learning_rates = [7.2e-4, 8.9e-4]
    regularization_strengths = [1e-3, 6.3e-3, 6.7e-3]
    # learning_rates = [7.2e-4]
    # regularization_strengths = [1e-3]
    for learning_rate in learning_rates:
        for reg_strength in regularization_strengths:
            net = TwoLayerNet(input_size, hidden_size, num_classes)
            net.train(X_train, y_train, X_val, y_val,
                num_iters = 2500, batch_size = 256,
